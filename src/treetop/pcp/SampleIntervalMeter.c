...
static const int SampleIntervalMeter_attributes[] = {
   METER_VALUE_OK
};

static void SampleIntervalMeter_updateValues(Meter* this) {
   int totalseconds = (int)Platform_getSampleInterval();
   if (totalseconds <= 0.0) {
...
const MeterClass SampleIntervalMeter_class = {
   .super = {
      .extends = Class(Meter),
      .delete = Meter_delete
   },
   .updateValues = SampleIntervalMeter_updateValues,
   .defaultMode = TEXT_METERMODE,
   .supportedModes = (1 << TEXT_METERMODE) | (1 << LED_METERMODE),
   .maxItems = 0,
   .total = 0.0,
   .attributes = SampleIntervalMeter_attributes,
   .name = "SampleInterval",
   .uiName = "SampleInterval",
   .caption = "Interval: "
};
