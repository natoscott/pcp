...
static const int TrainingTimeMeter_attributes[] = {
   METER_VALUE
};

static void TrainingTimeMeter_updateValues(Meter* this) {
   int totalseconds = (int)Platform_getTrainingTime();
   if (totalseconds <= 0.0) {
...
const MeterClass TrainingTimeMeter_class = {
   .super = {
      .extends = Class(Meter),
      .delete = Meter_delete
   },
   .updateValues = TrainingTimeMeter_updateValues,
   .defaultMode = TEXT_METERMODE,
   .supportedModes = (1 << TEXT_METERMODE) | (1 << LED_METERMODE),
   .maxItems = 0,
   .total = 0.0,
   .attributes = TrainingTimeMeter_attributes,
   .name = "TrainingTime",
   .uiName = "TrainingTime",
   .caption = "Training: "
};
