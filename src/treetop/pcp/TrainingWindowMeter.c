...
static const int TrainingWindowMeter_attributes[] = {
   METER_VALUE
};

static void TrainingWindowMeter_updateValues(Meter* this) {
   int totalseconds = (int)Platform_getTrainingWindow();
   if (totalseconds <= 0.0) {
...
const MeterClass TrainingWindowMeter_class = {
   .super = {
      .extends = Class(Meter),
      .delete = Meter_delete
   },
   .updateValues = TrainingWindowMeter_updateValues,
   .defaultMode = TEXT_METERMODE,
   .supportedModes = (1 << TEXT_METERMODE) | (1 << LED_METERMODE),
   .maxItems = 0,
   .total = 0.0,
   .attributes = TrainingWindowMeter_attributes,
   .name = "TrainingWindow",
   .uiName = "TrainingWindow",
   .caption = "Window: "
};
